...
def test(actor, env, seed=123, render=True, num_steps=100):
    ...
    for _ in range(num_steps):
        (
            ...
            _,
            i,
        ) = env.step(actor(o) * (high - low) / 2.0 + (high + low) / 2.0)
        if d:
            break
        os.append(o)
        rs.append(r)
...
def folder_to_results(env, render, num_tests, folder_path, steps=100, **kwargs):
    ...
    runs = np.array(
        list(
            map(
                lambda i: test(actor, env, seed=17 + i, render=render, num_steps=steps)[
                    1
                ],
                range(num_tests),
            )
...
def run_tests(env, cmd_args):
    ...
    print("################################")
    runs = [
        np.mean(folder_to_results(env, folder_path=folder, **vars(cmd_args)))
        for folder in folders
    ]
    return runs
