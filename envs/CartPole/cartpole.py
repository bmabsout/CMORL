...
def multi_dim_reward(state, action, env: "CartPoleEnv"):
    norm_cart_position = np.abs(state[0] / env.observation_space.high[0])
    norm_pole_angle = np.abs(state[2] / env.observation_space.high[2])
    norm_actuation = np.abs(action[0])
    pos_reward = 1 - norm_cart_position
    ang_reward = 1 - norm_pole_angle
    actuation_reward = 1 - norm_actuation
    rw_vec = np.array([pos_reward, ang_reward, actuation_reward], dtype=np.float32)
    return rw_vec


def sparse_multi_dim_reward(state, action, env: "CartPoleEnv"):
    norm_cart_position = np.abs(state[0] / env.observation_space.high[0])
    norm_pole_angle = np.abs(state[2] / env.observation_space.high[2])
    norm_actuation = np.abs(action[0])
    pos_reward = 1 - norm_cart_position
    ang_reward = 1 - norm_pole_angle
    actuation_reward = 1 - norm_actuation
    pos_reward = 1 if pos_reward > 0.9 else 0
    ang_reward = 1 if ang_reward > 0.9 else 0
    actuation_reward = 1 if actuation_reward > 0.9 else 0
    rw_vec = np.array([pos_reward, ang_reward, actuation_reward], dtype=np.float32)
    return rw_vec
...
def sparse_composed_reward_fn(state, action, env):
    rew_vec = sparse_multi_dim_reward(state, action, env)
    reward = p_mean(rew_vec, p=-4.0)
    return reward
    ...
    **Note:** While the ranges above denote the possible values for observation space of each element,
    ...
    def __init__(
        ...
        reward_dim = reward_fn(
            self.observation_space.sample()[0:3], self.action_space.sample(), self
        ).shape[0]
    ...
    def step(self, action):
        ...
        reward = self.cmorl(self.state, action, self)
